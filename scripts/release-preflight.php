...
passthru('cd ' . escapeshellarg($consumer) . ' && php artisan package:discover && php artisan migrate --force && php artisan capell:install --no-interaction --url=http://127.0.0.1:8099 --theme=none --name=Preflight --email=preflight@example.test --password=release-preflight-password --clear-cache --install-welcome-route', $exitCode);
if ($exitCode !== 0) {
    exit($exitCode);
}
