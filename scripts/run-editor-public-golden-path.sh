...
if [[ ! -f "${packages_root}/packages/discovery-foundation/composer.json" || ! -f "${packages_root}/packages/html-cache/composer.json" || ! -f "${packages_root}/packages/layout-builder/composer.json" || ! -f "${packages_root}/packages/navigation/composer.json" || ! -f "${packages_root}/packages/content-sections/composer.json" || ! -f "${packages_root}/packages/block-library/composer.json" ]]; then
    echo "CAPELL_PACKAGES_ROOT must point to a capell-packages checkout containing discovery-foundation, html-cache, layout-builder, navigation, content-sections, and block-library." >&2
    exit 2
fi
...
# fresh consumer solve the nine version-aligned split packages from the checked
...
composer config repositories.capell-layout-builder --json \
...
composer config repositories.capell-block-library --json \
    "$(path_repository_json capell-app/block-library "${packages_root}/packages/block-library")"
composer require \
    --no-interaction \
    --no-progress \
    ...
    capell-app/block-library:1.x-dev
