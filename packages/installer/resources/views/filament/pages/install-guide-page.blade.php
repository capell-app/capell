<x-filament-panels::page>
    ...
    @if (empty($data['patches']))
        <x-filament::section> {{ __('capell-installer::install-guide.no_patches_available') }} </x-filament::section>
    @else
                ...
                <x-filament::section>
                    ...
                    @foreach ($patches as $patch)
                        <x-filament::fieldset>
                            ...
                            <x-filament::badge> {{ $patch['status']->getLabel() }} </x-filament::badge>

                            {{ $patch['description'] }}

                            @if ($patch['reason'])
                                <x-filament::callout color="warning"> {{ $patch['reason'] }} </x-filament::callout>
                            @endif

                            @if ($patch['docUrl'])
                                <x-filament::button :href="$patch['docUrl']" tag="a" target="_blank" color="gray">
