<x-filament-panels::page>
    ...
    <div class="space-y-6">
        <x-filament::section>
            <x-slot name="heading">{{ __('capell-marketplace::marketplace.theme_extension.apply_heading') }}</x-slot>
            ...
            <div class="grid gap-4 lg:grid-cols-3">
                <label class="flex cursor-pointer gap-3 rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
                    <input type="radio" value="all" wire:model.live="scope" class="mt-1" />
                    <span>
                ...
                <label class="flex cursor-pointer gap-3 rounded-lg border border-gray-200 bg-white p-4 shadow-sm lg:col-span-2 dark:border-white/10 dark:bg-gray-900">
                    <input type="radio" value="site" wire:model.live="scope" class="mt-1" />
                    <span class="w-full">
                        <span class="block text-sm font-medium text-gray-950 dark:text-white">
            ...
            <div class="mt-5 flex flex-wrap gap-3">
                <x-filament::button color="gray" icon="heroicon-o-eye" wire:click="previewTheme">
                    {{ __('capell-marketplace::marketplace.theme_extension.preview_button', ['theme' => $this->themeName()]) }}
                </x-filament::button>

                <x-filament::button icon="heroicon-o-check-circle" wire:click="applyTheme">
                ...
                @foreach ($sites as $site)
                    <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <h3 class="text-sm font-medium text-gray-950 dark:text-white">{{ $site->name }}</h3>
                            ...
                            @if ($site->theme?->key === $this->themeKey)
                                <span class="rounded bg-emerald-50 px-2 py-1 text-xs font-medium text-emerald-700 ring-1 ring-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-100 dark:ring-emerald-400/40">
