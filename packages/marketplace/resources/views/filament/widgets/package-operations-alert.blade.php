            ...
            <div class="space-y-4">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @if ($activeCount > 0)
                            <span class="bg-info-50 text-info-700 ring-info-600/20 dark:bg-info-500/10 dark:text-info-300 rounded-md px-2.5 py-1 text-xs font-semibold ring-1">
                                {{ trans_choice('capell-marketplace::marketplace.operations.active_count', $activeCount, ['count' => $activeCount]) }}
                            </span>
                        @endif

                        @if ($attentionCount > 0)
                            <span class="bg-danger-50 text-danger-700 ring-danger-600/20 dark:bg-danger-500/10 dark:text-danger-300 rounded-md px-2.5 py-1 text-xs font-semibold ring-1">
                                    ...
                                    <div class="min-w-0">
                                        <div class="flex flex-wrap items-center gap-2">
                                            ...
                                            @if ($operation->failure_stage)
                                                <span class="rounded-md bg-white/80 px-2 py-1 text-xs font-medium text-gray-700 ring-1 ring-gray-600/20 dark:bg-white/10 dark:text-gray-200">
