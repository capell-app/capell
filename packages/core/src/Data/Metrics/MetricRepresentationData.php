...
final class MetricRepresentationData extends Data
{
    public function __construct(
        ...
        if ($this->valueType !== $expectedValueType) {
            throw new InvalidArgumentException('Metric unit and value type are incompatible.');
        }

        if ($this->valueType === MetricValueType::Integer && ($this->scale !== null || $this->currency !== null)) {
            throw new InvalidArgumentException('Integer metric representations cannot define scale or currency.');
        }

        if ($this->valueType === MetricValueType::Decimal
            && ($this->scale === null || $this->scale < 0 || $this->scale > 18 || $this->currency !== null)) {
            throw new InvalidArgumentException('Decimal metric representations require a fixed scale without currency.');
        }

        if ($this->valueType === MetricValueType::MinorCurrencyUnit
            && ($this->scale === null || $this->scale < 0 || $this->scale > 18 || preg_match('/\A[A-Z]{3}\z/', $this->currency ?? '') !== 1)) {
            throw new InvalidArgumentException('Currency metric representations require fixed uppercase currency and scale.');
        }
    }
