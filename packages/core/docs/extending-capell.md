...
Treat a service lifetime as part of its public contract: registrations collected while providers boot may live for the worker lifetime, but anything derived from the current request or operation must be scoped. Use a reset-tagged singleton only when it genuinely has to preserve boot metadata as well as temporary operation state. Never register the same service as both scoped and resettable. Capell's executable inventory lives in `SingletonLifetimeInventory`; new mutable singletons must be classified there with an exact concrete class and reason.
