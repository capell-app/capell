...
@if ($hasStructuredShape)
    <dl class="grid grid-cols-1 gap-3 text-sm sm:grid-cols-3 md:grid-cols-4">
        <div class="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
            <dt class="text-xs text-gray-500 uppercase">{{ __('capell-admin::exchanger.result_pages_imported') }}</dt>
            <dd class="font-mono text-lg">{{ $pagesImported }}</dd>
        </div>
        <div class="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
            <dt class="text-xs text-gray-500 uppercase">{{ __('capell-admin::exchanger.result_pages_skipped') }}</dt>
        ...
        <div class="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
            <dt class="text-xs text-gray-500 uppercase">{{ __('capell-admin::exchanger.result_media_ingested') }}</dt>
    ...
    @endif
@else
    <pre class="overflow-auto rounded bg-gray-50 p-3 text-xs dark:bg-gray-900"><code>{{ json_encode($resultSummary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</code></pre>
@endif
