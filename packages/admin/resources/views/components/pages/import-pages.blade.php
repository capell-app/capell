<x-filament-panels::page>
    ...
    @elseif ($step === 'review')
        <div class="space-y-4">
            <div>
                <h2 class="text-lg font-semibold">{{ __('capell-admin::exchanger.review_heading') }}</h2>
            ...
            <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            <th class="px-3 py-2 text-left text-xs font-medium tracking-wider uppercase">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
                        @foreach ($reviewRows as $reviewRow)
                            ...
                            <tr>
                                <td class="px-3 py-2 text-sm">{{ $reviewRow['title'] ?? '—' }}</td>
                                <td class="px-3 py-2 font-mono text-sm">{{ $reviewRow['primary_url'] ?? '—' }}</td>
                                ...
                                <td class="px-3 py-2 text-sm">
                                    <span class="{{ $badgeClasses }} inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium">
                                        {{ $collisionLabel }}
                                    </span>
                                </td>
                                <td class="px-3 py-2 text-sm">
                                    ...
                                    >
                                        ...
                                        <option value="skip">{{ __('capell-admin::exchanger.action_skip') }}</option>
                                    </select>
                                </td>
                            </tr>
                        @endforeach
    ...
    @elseif ($step === 'resolve')
        ...
        <div class="space-y-6">
            <div>
                <h2 class="text-lg font-semibold">{{ __('capell-admin::exchanger.resolve_heading') }}</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">
            ...
            @foreach ($groupedRows as $groupKey => $rows)
                <div class="space-y-2">
                    <h3 class="text-sm font-semibold tracking-wider text-gray-700 uppercase dark:text-gray-200">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
                                @foreach ($rows as $resolveRow)
                                    ...
                                    <tr>
                                        <td class="px-3 py-2 font-mono text-sm">{{ $rowRef }}</td>
                                        <td class="px-3 py-2 text-sm">
                                            @if ($topMatch)
                                                <div class="font-medium">#{{ $topMatch['local_id'] }}</div>
                                                <div class="text-xs text-gray-500">
                                                    {{ $topMatch['strategy'] }} · {{ number_format((float) $topMatch['confidence'] * 100, 0) }}%
                                                </div>
                                                @if (! empty($topMatch['reason']))
                                                    <div class="text-xs text-gray-400">{{ $topMatch['reason'] }}</div>
                                                @endif
                                            @else
            ...
            <div class="flex items-center gap-3">
                ...
                <x-filament::button color="gray" wire:click="backToReview">
                    {{ __('capell-admin::exchanger.back_button') }}
                </x-filament::button>
            </div>
        </div>
    @elseif ($step === 'validate')
        ...
        <div class="space-y-6">
            <div>
                <h2 class="text-lg font-semibold">{{ __('capell-admin::exchanger.validate_heading') }}</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">
            ...
            @if ($hasBlockingErrors)
                <div class="rounded-lg border border-rose-200 bg-rose-50 p-4 dark:border-rose-900 dark:bg-rose-950">
            ...
            <div class="space-y-2">
                <label for="import-confirmation" class="block text-sm font-medium">
            ...
            <div class="flex items-center gap-3">
                ...
                <x-filament::button color="gray" wire:click="backToResolve">
                    {{ __('capell-admin::exchanger.back_button') }}
                </x-filament::button>
            </div>
        </div>
    @elseif ($step === 'executing')
        ...
        <div class="space-y-4" wire:poll.2s="refreshStatus">
            <div>
                <h2 class="text-lg font-semibold">{{ __('capell-admin::exchanger.execute_heading') }}</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">
            ...
            <div>
                <div class="mb-1 flex justify-between text-xs text-gray-500">
                    <span> {{ __('capell-admin::exchanger.progress_label') }} </span>
                ...
                >
                    <div class="bg-primary-600 h-full transition-all" style="width: {{ $progressPercent }}%"></div>
                </div>
            </div>
        </div>
    @elseif ($step === 'completed')
        ...
        <div class="space-y-4 rounded-lg border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900 dark:bg-emerald-950">
            <h2 class="text-lg font-semibold">{{ __('capell-admin::exchanger.status_completed') }}</h2>
            ...
            <div class="flex items-center gap-3">
                @if ($workspaceUrl)
                    <x-filament::button tag="a" :href="$workspaceUrl">
    ...
    @elseif ($step === 'failed')
        <div class="space-y-4 rounded-lg border border-rose-200 bg-rose-50 p-4 dark:border-rose-900 dark:bg-rose-950">
            ...
            @if (! empty($failureReason))
                <pre class="overflow-auto rounded bg-rose-100 p-3 text-xs text-rose-900 dark:bg-rose-900/40 dark:text-rose-100">{{ $failureReason }}</pre>
            @endif
