...
@if ($languageOptions->isNotEmpty())
    ...
    >
        ...
        <div class="fi-dropdown-list-item fi-dropdown-list-item-color-gray flex flex-col items-stretch gap-1.5 rounded-md p-2">
            <label for="capell-admin-user-menu-language" class="text-xs font-medium text-gray-500 dark:text-gray-400">
            ...
            >
                <option value="">{{ __('capell-admin::form.default_locale') }}</option>

                @foreach ($languageOptions as $languageId => $languageName)
                    <option value="{{ $languageId }}" @selected($selectedLanguageId === $languageId)>
                        {{ $languageName }}
                    </option>
                @endforeach
            </select>
