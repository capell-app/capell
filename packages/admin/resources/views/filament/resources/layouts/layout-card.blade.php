...
<div class="group overflow-hidden rounded-lg transition duration-150 hover:-translate-y-0.5">
    <div class="relative aspect-[16/10] w-full overflow-hidden bg-gray-100 dark:bg-gray-950">
        @if ($card->imageUrl !== null)
            ...
            />
        @else
            <div class="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_top_left,rgba(var(--primary-500),0.18),transparent_34%),linear-gradient(135deg,rgba(17,24,39,0.04),rgba(17,24,39,0.12))] px-6 text-center dark:bg-[radial-gradient(circle_at_top_left,rgba(var(--primary-400),0.22),transparent_34%),linear-gradient(135deg,rgba(255,255,255,0.06),rgba(255,255,255,0.02))]">
                <div class="max-w-52">
                    <div class="text-[0.68rem] font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">
                        {{ __('capell-admin::table.layout_preview_fallback') }}
                    </div>
                    <div class="mt-2 truncate text-xl font-semibold text-gray-950 dark:text-white">
                        {{ $card->title }}
                    </div>
                </div>
            </div>
        ...
        <div class="absolute right-3 bottom-3 left-3 flex items-end justify-between gap-3">
            ...
            @if ($card->isDefault)
                <span class="bg-primary-500 shrink-0 rounded-md px-2 py-1 text-xs font-semibold text-white shadow-sm">
    ...
    <div class="space-y-4 p-4">
        <div class="min-h-8 space-y-1 text-sm leading-6 text-gray-600 dark:text-gray-300">
        ...
        <details class="group/details border-t border-gray-100 pt-3 dark:border-white/10">
            <summary class="flex cursor-pointer list-none items-center justify-between gap-3 text-sm font-medium text-gray-700 marker:hidden dark:text-gray-200">
                <span class="flex items-center gap-2">
                    <span class="bg-primary-50 text-primary-600 dark:bg-primary-400/10 dark:text-primary-300 grid size-5 place-items-center rounded-md text-xs font-semibold">
                        {{ $card->containerCount }}
                    </span>
                    <span> {{ __('capell-admin::table.layout_containers') }} </span>
                </span>
            ...
            <div class="mt-3 grid gap-2 text-sm sm:grid-cols-2">
                @forelse ($card->containerNames as $containerName)
                    <div class="flex items-center gap-2 rounded-md bg-gray-50 px-3 py-2 text-gray-700 dark:bg-white/5 dark:text-gray-200">
                        <span class="bg-primary-500 size-1.5 shrink-0 rounded-full"></span>
        ...
        <div class="flex items-center justify-between border-t border-gray-100 pt-3 text-xs dark:border-white/10">
