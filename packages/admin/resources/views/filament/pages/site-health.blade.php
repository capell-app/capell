...
<x-filament-panels::page>
    <div class="space-y-6">
        <div class="flex justify-end">
            <div class="w-full max-w-xs">
                ...
                <x-filament::input.wrapper>
                    <x-filament::input.select id="site-health-site-selector" wire:model.live="selectedSiteId">
                        @forelse ($siteOptions as $siteId => $siteName)
                            <option value="{{ $siteId }}">{{ $siteName }}</option>
                        @empty
                            <option value="">{{ __('capell-admin::generic.no_sites') }}</option>
                        @endforelse
        ...
        <div class="grid gap-6 lg:grid-cols-2" wire:loading.attr="aria-busy" wire:target="selectedSiteId">
                            ...
                            <li class="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
                                <div class="flex min-w-0 items-start justify-between gap-3">
                                    <div class="min-w-0">
                                        ...
                                        <div class="mt-1 text-sm [overflow-wrap:anywhere] break-words text-gray-600 dark:text-gray-400">
                                            {{ $check->detail }}
                                        </div>
                                    </div>
                                    <x-filament::badge :color="$badgeColor($check->status)" class="shrink-0">
                                        <span class="sr-only"> {{ __('capell-admin::generic.status') }}: </span>
                                ...
                                @if ($check->path || $check->generatedAt || $check->remediation)
                                    <div class="mt-3 space-y-1 text-xs text-gray-500 dark:text-gray-400">
                                        @if ($check->path)
                                            <div class="overflow-x-auto font-mono break-all">{{ $check->path }}</div>
                                        @endif
