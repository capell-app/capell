...
<x-filament-panels::page>
    <div class="space-y-5">
        <section class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-900">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <dl class="grid gap-2 sm:grid-cols-4 lg:min-w-[34rem]">
                    <div class="rounded-md border border-gray-200 p-3 dark:border-white/10">
                        ...
                        <dd class="text-danger-600 dark:text-danger-300 mt-1 text-lg font-semibold">
                    ...
                    <div class="rounded-md border border-gray-200 p-3 dark:border-white/10">
                        ...
                        <dd class="mt-1 text-lg font-semibold">{{ $summaryCounts['bugfix'] }}</dd>
                    </div>
                    <div class="rounded-md border border-gray-200 p-3 dark:border-white/10">
                        <dt class="text-xs text-gray-500 dark:text-gray-400">
                            {{ __('capell-admin::generic.feature') }}
                        </dt>
                        <dd class="mt-1 text-lg font-semibold">{{ $summaryCounts['feature'] }}</dd>
                    </div>
                    <div class="rounded-md border border-gray-200 p-3 dark:border-white/10">
                        <dt class="text-xs text-gray-500 dark:text-gray-400">
                            {{ __('capell-admin::generic.major') }}
                        </dt>
                        <dd class="text-warning-600 dark:text-warning-300 mt-1 text-lg font-semibold">
                            {{ $summaryCounts['major'] }}
                        </dd>
                    </div>
                </dl>
        ...
        <dl class="grid gap-3 md:grid-cols-4">
            <div class="rounded-lg border border-gray-200 p-3 dark:border-white/10">
                ...
                <dd class="mt-1 text-sm font-medium">{{ $this->installedCapellVersion() }}</dd>
            </div>
            <div class="rounded-lg border border-gray-200 p-3 dark:border-white/10">
                <dt class="text-xs text-gray-500 dark:text-gray-400">
                    {{ __('capell-admin::generic.target_version') }}
                </dt>
                <dd class="mt-1 text-sm font-medium">{{ $this->targetCapellVersion() }}</dd>
            </div>
            <div class="rounded-lg border border-gray-200 p-3 dark:border-white/10">
                <dt class="text-xs text-gray-500 dark:text-gray-400">{{ __('capell-admin::generic.last_checked') }}</dt>
                <dd class="mt-1 text-sm font-medium">
                    {{ $latestAdvisorySnapshot?->checked_at?->diffForHumans() ?? __('capell-admin::generic.never') }}
                </dd>
            </div>
            <div class="rounded-lg border border-gray-200 p-3 dark:border-white/10">
                <dt class="text-xs text-gray-500 dark:text-gray-400">{{ __('capell-admin::generic.last_run') }}</dt>
                <dd class="mt-1 text-sm font-medium">
        ...
        <section class="space-y-3">
            <div class="flex items-center justify-between gap-3">
                <h2 class="text-base font-semibold">{{ __('capell-admin::generic.updates_to_review') }}</h2>
                <a
                            ...
                            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                <div class="min-w-0">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <span class="rounded-full bg-gray-100 px-2 py-1 text-xs font-semibold text-gray-700 uppercase dark:bg-white/10 dark:text-gray-200">
                                ...
                                <dl class="grid shrink-0 grid-cols-2 gap-3 text-sm md:w-64">
                                    <div>
                                        ...
                                        <dd class="mt-1 font-medium">{{ $this->noticeImpactLabel($notice) }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-xs text-gray-500 dark:text-gray-400">
                                            {{ __('capell-admin::generic.backup') }}
                                        </dt>
                                        <dd class="mt-1 font-medium">{{ __('capell-admin::generic.recommended') }}</dd>
                                    </div>
                                </dl>
                            </div>
            ...
            <div class="space-y-3">
                ...
                <dl class="grid gap-3 md:grid-cols-2">
                    <div>
                        ...
                        <dd class="mt-1 overflow-auto rounded-md bg-gray-950 px-3 py-2 font-mono text-xs text-gray-100">
        ...
        <section class="overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-white/10 dark:bg-gray-900">
            <div class="flex flex-col gap-3 border-b border-gray-200 bg-gray-50/80 px-4 py-3 md:flex-row md:items-center md:justify-between dark:border-white/10 dark:bg-white/[0.03]">
                <div class="flex min-w-0 items-baseline gap-3">
                    <h2 class="shrink-0 text-sm font-semibold text-gray-950 dark:text-white">
            ...
            <ol class="grid gap-px bg-gray-200 md:grid-cols-2 xl:grid-cols-3 dark:bg-white/10">
                @foreach ($readinessReport->checks as $stepIndex => $check)
                    <li class="relative bg-white px-4 py-3 dark:bg-gray-900">
                        <div class="flex items-start gap-2.5">
                            <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-gray-100 text-xs font-semibold text-gray-700 dark:bg-white/10 dark:text-gray-200">
            ...
            <section class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-900">
                ...
                <ol class="mt-4 space-y-3">
                    @forelse ($recentUpgradeRunEvents as $event)
                        <li class="border-l-2 border-gray-200 pl-3 dark:border-white/10">
                            <div class="flex flex-wrap items-center gap-2">
        ...
        @if ($this->lastOutput !== null)
            ...
            >
                ...
                <pre class="mt-3 max-h-96 overflow-auto rounded-lg bg-gray-950 p-4 text-sm text-gray-100">{{ $this->lastOutput }}</pre>
            </details>
        @endif
    </div>
