    ...
    <dl class="grid gap-3 sm:grid-cols-2">
        @foreach ($details as $detailLabel => $detailValue)
            @if ($detailValue !== null && $detailValue !== '')
                <div class="rounded-lg border border-gray-200 bg-gray-50/70 p-3 dark:border-white/10 dark:bg-white/5">
                    <dt class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ $detailLabel }}</dt>
                    <dd class="mt-1 text-sm font-semibold text-gray-950 dark:text-white">{{ $detailValue }}</dd>
                </div>
            @endif
        @endforeach

        @if ($updateAvailable && is_string($latestVersion) && $latestVersion !== '')
            <div class="border-warning-200 bg-warning-50 dark:border-warning-400/20 dark:bg-warning-400/10 rounded-lg border p-3">
                <dt class="text-warning-700 dark:text-warning-200 text-xs font-medium">
                    {{ __('capell-admin::table.latest_version') }}
                </dt>
                <dd class="text-warning-800 dark:text-warning-100 mt-1 text-sm font-semibold">
