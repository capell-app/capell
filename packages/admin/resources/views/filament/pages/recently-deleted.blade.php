<x-filament-panels::page>
    <div class="space-y-8">
        @foreach ($groups as $group)
            <div>
                <h2 class="flex items-center gap-2 text-base font-semibold text-gray-950 dark:text-white">
                    <x-filament::icon :icon="$group['icon']" class="h-5 w-5 text-gray-400" />
                    {{ $group['label'] }}
                    <span class="text-sm font-normal text-gray-500"> ({{ $group['items']->count() }}) </span>
                </h2>

                @if ($group['items']->isEmpty())
                    <p class="mt-2 text-sm text-gray-500">{{ __('capell-admin::generic.recently_deleted_none') }}</p>
                @else
                    <ul class="mt-3 divide-y divide-gray-100 rounded-lg border border-gray-200 bg-white dark:divide-white/5 dark:border-white/10 dark:bg-gray-900">
                        @foreach ($group['items'] as $item)
                            <li class="flex items-center justify-between gap-4 px-4 py-3">
                                ...
                                <div class="flex shrink-0 items-center gap-2">
                                    ...
                                    >
                                        <x-filament::icon icon="heroicon-m-arrow-uturn-left" class="h-3.5 w-3.5" />
