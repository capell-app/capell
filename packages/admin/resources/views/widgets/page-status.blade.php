<x-filament-widgets::widget>
    <x-filament::section :heading="__('capell-admin::dashboard.widget_capell_overview')">
        <div class="@container">
            <div class="@md:grid-cols-2 grid gap-2 @4xl:grid-cols-3">
                @foreach ($this->stats as $stat)
                    ...
                    @else
                        <div class="flex min-h-20 items-start justify-between gap-3 rounded-lg border border-gray-200 bg-gray-50/70 px-3 py-2.5 dark:border-white/10 dark:bg-white/5">
                            @include('capell-admin::widgets.partials.capell-overview-stat-row', ['stat' => $stat])
                        </div>
                    @endif
                @endforeach
