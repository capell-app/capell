        ...
        <div class="space-y-4">
            ...
            @if ($data->isAllGreen())
                <div class="flex items-center gap-2 rounded-lg bg-green-50 px-3 py-2 dark:bg-green-900/20">
                    ...
                    <span class="text-sm font-medium text-green-700 dark:text-green-300"> All migrated </span>
                </div>
            @else
                {{-- Pending migrations --}}
                @if ($data->pendingCount > 0)
                    <details class="group" open>
                        <summary class="flex cursor-pointer items-center justify-between gap-2 py-1 text-sm font-medium">
                        ...
                        <div class="mt-2 space-y-1 pl-4">
                            @foreach ($data->pendingMigrations as $migration)
                                <div class="rounded px-2 py-1 font-mono text-xs text-gray-700 odd:bg-gray-50 dark:text-gray-200 dark:odd:bg-gray-800/50">
                                    {{ $migration }}
                                </div>
                            @endforeach
                        </div>
                    ...
                    <details class="group" open>
                        ...
                        <div class="mt-2 space-y-2 pl-4">
                            @foreach ($data->orphanedRegistrations as $orphan)
                                <div class="rounded px-2 py-1.5 text-xs odd:bg-gray-50 dark:odd:bg-gray-800/50">
                                    ...
                                    <div class="mt-0.5 truncate font-mono text-gray-400 dark:text-gray-500">
                                        {{ $orphan['expectedPath'] }}
                                    </div>
                                </div>
                            @endforeach
