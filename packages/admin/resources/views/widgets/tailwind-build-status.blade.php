        ...
        <div class="space-y-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                @if ($data->freshCount > 0)
                    <span class="rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-semibold text-green-700 dark:bg-green-900/40 dark:text-green-300">
                ...
                @if ($data->sites->count() === 0)
                    <span class="text-xs text-gray-400 dark:text-gray-500"> No sites found. </span>
                @endif
            </div>
            ...
            @if ($data->sites->count() > 0)
                <div class="space-y-1.5">
                    @foreach ($data->sites as $siteRow)
                        <div class="flex items-center gap-2 rounded px-2 py-1.5 text-xs odd:bg-gray-50 dark:odd:bg-gray-800/50">
                            <span class="flex-1 font-medium text-gray-700 dark:text-gray-200">
                                {{ $siteRow->siteName }}
                            </span>

                            @if ($siteRow->status === 'fresh')
                                <span class="shrink-0 rounded bg-green-100 px-1.5 py-0.5 text-green-700 dark:bg-green-900/40 dark:text-green-300">
                                    fresh
                                </span>
