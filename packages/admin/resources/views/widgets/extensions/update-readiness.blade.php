    ...
    >
        <div class="space-y-2">
            @foreach (collect($this->updates)->reject(fn ($update) => $update->state === 'none')->take(6) as $update)
                <div class="flex items-center justify-between gap-3 rounded-lg border border-gray-200 bg-gray-50/70 px-3 py-2.5 text-sm dark:border-white/10 dark:bg-white/5">
                    <div class="min-w-0">
