...
@if ($view->isStatusable())
    <div class="flex items-center gap-2">
        ...
        <span class="{{ $labelClass }}"> {{ __('capell-admin::publish_panel.status') }} </span>
...
@if ($view->isPublishable())
    ...
    <div class="flex items-center gap-2">
        ...
        <span class="{{ $labelClass }}"> {{ __('capell-admin::publish_panel.published_label') }} </span>
        @if ($view->publishedAt !== null)
            <span class="{{ $valueClass }}" title="{{ $view->publishedAt->translatedFormat($titleFormat) }}">
    ...
    @if (! $view->isLive() && ! $view->isExpired())
        <div class="flex items-center gap-2">
            ...
            <span class="{{ $labelClass }}"> {{ __('capell-admin::publish_panel.goes_live') }} </span>
            <span class="ml-auto inline-flex items-center gap-2">
                ...
                @if ($this->canManage())
                    <span class="-my-1"> {{ $this->schedulePublishAction }} </span>
                @endif
            </span>
        </div>
    @endif
    ...
    @if ($view->isLive() || $view->isScheduled())
        <div class="flex items-center gap-2">
            ...
            <span class="{{ $labelClass }}"> {{ __('capell-admin::publish_panel.expires') }} </span>
            <span class="ml-auto inline-flex items-center gap-2">
    ...
    @if ($view->isExpired() && $view->expiresAt !== null)
        <div class="flex items-center gap-2">
            ...
            <span class="{{ $labelClass }}"> {{ __('capell-admin::publish_panel.unpublished_on') }} </span>
            <span class="{{ $valueClass }}" title="{{ $view->expiresAt->translatedFormat($titleFormat) }}">
                {{ $view->expiresAt->translatedFormat($displayFormat) }}
            </span>
        </div>
    @endif
