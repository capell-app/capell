...
<div class="space-y-4 text-sm">
    ...
    @if ($blockingIssues !== [])
        <div class="border-danger-300 bg-danger-50 dark:border-danger-700/50 dark:bg-danger-900/20 rounded-lg border p-3">
            <p class="text-danger-700 dark:text-danger-300 font-semibold">
                {{ __('capell-admin::event-sourcing.rollback_blocked') }}
            </p>
            <ul class="text-danger-700 dark:text-danger-300 mt-2 list-disc space-y-1 ps-5">
    ...
    @if ($warnings !== [])
        <div class="border-warning-300 bg-warning-50 dark:border-warning-700/50 dark:bg-warning-900/20 rounded-lg border p-3">
            <ul class="text-warning-700 dark:text-warning-300 list-disc space-y-1 ps-5">
                ...
                <div class="py-3">
                    ...
                    <div class="mt-1 grid gap-2 sm:grid-cols-2">
                        <div class="rounded bg-gray-50 p-2 dark:bg-white/5">
                            <span class="block text-xs text-gray-500 uppercase dark:text-gray-300">
                                {{ __('capell-admin::event-sourcing.rollback_current') }}
                            </span>
                            <span class="text-gray-700 dark:text-gray-200"> {{ $field->beforeSummary }} </span>
                        </div>
                        <div class="bg-primary-50 dark:bg-primary-900/20 rounded p-2">
                            <span class="block text-xs text-gray-500 uppercase dark:text-gray-300">
                                {{ __($direction->targetColumnKey()) }}
                            </span>
                            <span class="text-gray-700 dark:text-gray-200"> {{ $field->afterSummary }} </span>
                        </div>
                    </div>
                </div>
            @endforeach
