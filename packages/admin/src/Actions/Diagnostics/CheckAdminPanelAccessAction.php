    ...
    public function handle(): DoctorCheckResultData
    {
        ...
        foreach ($users as $user) {
            ...
            try {
                ...
                // A broken role/guard/pivot configuration proves no access.
            }
        }
        ...
        if (is_string($roleModel) && is_a($roleModel, Model::class, true)) {
            ...
            if (Schema::hasTable($roleTable)) {
                $roleIds = $roleModel::query()->where('name', $roleName)->where('guard_name', $guard)->pluck($rolePrototype->getKeyName());
                $roleCount = $roleIds->count();
                if (Schema::hasTable($pivotTable) && $roleIds->isNotEmpty()) {
                    $assignmentCount = DB::table($pivotTable)->whereIn('role_id', $roleIds)->where('model_type', $userMorphType)->count();
                }
            }
        }

        $evidence = [...$baseEvidence, 'user_count' => count($users), 'matching_role_count' => $roleCount, 'matching_assignment_count' => $assignmentCount, 'accessible_user_count' => $accessibleUsers, 'panel_registered' => $panelRegistered];
        ...
        return new DoctorCheckResultData('Admin user access', true, sprintf('%d configured user(s) have effective admin panel access.', $accessibleUsers), id: 'core.admin.access', severity: DoctorCheckSeverity::Critical, evidence: $evidence);
    }

    /** @param array<string, mixed> $evidence */
    private function failed(string $message, array $evidence): DoctorCheckResultData
    {
        return new DoctorCheckResultData('Admin user access', false, $message, 'Create a user with the configured guard and grant effective access to the Filament admin panel.', id: 'core.admin.access', severity: DoctorCheckSeverity::Critical, evidence: $evidence);
    }
}
