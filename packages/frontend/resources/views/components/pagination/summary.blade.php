...
<div {{
    ...
    @if ($results->perPage() < $results->total())
        ...
        <span class="pagination-range font-semibold tracking-normal dark:text-white">
        ...
        <span class="pagination-total font-semibold tracking-normal dark:text-white"> {{ $results->total() }} </span>
        {{ $resultsFoundText }}
    @else
        {{ __('capell-frontend::messages.showing') }}
        <span class="pagination-total font-semibold tracking-normal dark:text-white"> {{ $results->total() }} </span>
