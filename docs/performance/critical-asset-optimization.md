...
Capell treats the page image as the conservative LCP candidate when that relation is already loaded. The frontend render data keeps the media's canonical URL for identity and emits a responsive image preload when conversions or Media Library responsive sources are available. The preload uses the selected conversion's MIME type. This lets the browser select the viewport-appropriate candidate while the rendered image still receives `fetchpriority="high"` and eager loading even when its fallback `src` uses a different conversion.

The shared media component defaults other images to native lazy loading. Pass `loading="eager"` explicitly for additional above-the-fold media that is not the page image. The logo component does this automatically. Video preload behaviour is unchanged.

Local theme fonts generated by the public head use `font-display: swap`. URL-based font stylesheets remain controlled by their provider.
