...
Capell packages are grouped by customer-facing value, not by internal implementation detail. The Composer package names remain stable, while package manifests expose `product.group`, `product.tier`, and `product.bundle` for catalogue, marketplace, and pricing screens.
...
| Site Stats         | `capell-app/site-stats`         | Privacy-safe page and site totals provide useful growth trends without adding visitor tracking.                       |
...
Use `product.tier: "free"` for Capell Foundation packages and explicitly free support packages. Use `product.tier: "premium"` for paid product-group packages.

The `product.bundle` value is a stable machine key. The `product.group` value is display copy and may appear in marketplace, installer, and documentation surfaces.
