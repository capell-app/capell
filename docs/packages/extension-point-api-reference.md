...
| Role workspace tool                    | `AdminBridgeRegistrar::workspace(AdminWorkspaceItemData $item)`                                                                   | Admin bridge or runtime/admin provider | Header workspace switcher          | Item hidden                           | Register a translated label, a typed Page/Resource URL with `isAbsolute: false`, and the required permission; assert the bridge definition and visible filtered result. |
...
Workspace tool URLs are a discovery-only contract and must resolve to root-relative, same-origin paths. Use `Page::getUrl(..., isAbsolute: false)`, `Resource::getUrl(..., isAbsolute: false)`, or `route(..., absolute: false)` when those typed targets exist. The registry rejects absolute, protocol-relative, control-character, and backslash-containing destinations; direct Page/Resource policies remain authoritative.
