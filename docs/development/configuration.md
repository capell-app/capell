...
| `CAPELL_ANALYTICS_VISITOR_RETENTION_DAYS`    | `30`                                      | Age, clamped to 14–400, after which visitor-day markers are pruned by the daily `capell:activity:prune-visitors` schedule                                           |
...
| `CAPELL_PLUGINS_RETRY_TIMES`                 | `3`                                       | Maximum attempts for the idempotent plugin catalogue read                                                                                                           |
| `CAPELL_PLUGINS_RETRY_DELAY_MS`              | `500`                                     | Base delay in milliseconds between plugin catalogue read attempts                                                                                                   |
| `CAPELL_PLUGINS_RETRY_AFTER_MAX_MS`          | `60000`                                   | Maximum accepted Retry-After delay in milliseconds for plugin catalogue reads                                                                                       |
| `CAPELL_MARKETPLACE_READ_RETRY_TIMES`        | `3`                                       | Maximum attempts for idempotent Marketplace catalogue and extension reads                                                                                           |
| `CAPELL_MARKETPLACE_READ_RETRY_DELAY_MS`     | `500`                                     | Base delay in milliseconds between Marketplace read attempts                                                                                                        |
| `CAPELL_MARKETPLACE_READ_RETRY_AFTER_MAX_MS` | `60000`                                   | Maximum accepted Retry-After delay in milliseconds for Marketplace reads                                                                                            |
...
Visitor-day markers are separate rows with their own schedule, `capell:activity:prune-visitors` at 00:45 UTC, and their own window: `CAPELL_ANALYTICS_VISITOR_RETENTION_DAYS` is clamped to 14–400 and has no Admin settings equivalent. The floor of 14 is deliberate — the dashboard compares the last seven days against the seven before them, and a multi-day visitor count cannot be rebuilt from daily rollups once the rows are gone. Each marker is a truncated HMAC of site, address and user agent, salted with the application key and the UTC day; the daily rotation means the same person is unlinkable across days, so a multi-day figure counts a returning visitor once per day.
