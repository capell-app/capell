...
[![First-party packages](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fraw.githubusercontent.com%2Fcapell-app%2Fcapell%2Fmain%2Fdocs%2Freference%2Freadme-engineering-metrics.json&query=%24.package_count&label=first-party%20packages&style=flat-square&color=2F855A)](docs/packages/catalog.md)
[![Core Pest tests](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fraw.githubusercontent.com%2Fcapell-app%2Fcapell%2Fmain%2Fdocs%2Freference%2Freadme-engineering-metrics.json&query=%24.test_count&label=Core%20Pest%20tests&style=flat-square&color=2F855A)](docs/standards/coding-standards.md)
...
The package and test badges are generated by
`CAPELL_FIRST_PARTY_CATALOGUE_TEST=/path/to/capell-app/tests/Feature/Marketplace/FirstPartyPackageCatalogueTest.php php scripts/check-readme-engineering-standards.php --update` from the
Marketplace first-party catalogue assertion and the Core Pest suite. The
committed [metrics source](docs/reference/readme-engineering-metrics.json) is
checked by `composer run check:readme-engineering-standards`; these are count
snapshots, not adoption or quality guarantees.
...
![Conceptual page editing workflow from Admin scratch recovery and optional private previews through validated saves, revision history, publication visibility, and public rendering](docs/images/capell-page-editing-workflow.svg)
...
### Add behaviour through contracts

An extension package owns its feature code and contributes through its manifest-scoped providers. The Foundation registrars compose Core, Admin/editor, and Frontend/public surfaces; the Extension Cookbook is a concrete example of a page-schema extension and typed frontend registrations.
...
![Page-centred domain map connecting Site, Blueprint, Layout, Theme, Translation, PageUrl, Language, and SiteDomain](docs/images/capell-page-identity-graph.svg)

_The Page holds identity and hierarchy. Its blueprint defines structure, its translations hold localised content, its URLs locate it, and its layout connects content to presentation._

[Canonical Mermaid source](docs/images/capell-page-identity-graph.mmd) · [Detailed Core relationship map](docs/reference/core-erd.md)
